...
void *newThread(void *dummy)
{
  pthread_t mythid;
  //
  // Note that mythid variable is a dynamic local variable of mythread () funct>  // that is, it is pushed on the stack and, therefore, is not shared by the th>  //
  mythid = pthread_self();

  a = a+1;
  printf("Thread new %u, Calculation result = %d\n", mythid, a);
return NULL;
}
...
int main()
{
  ...
  resultNew = pthread_create (&thidNew, (pthread_attr_t *)NULL, newThread, NULL);
  ...
  printf("New thread created, thid = %u\n", thidNew);
  ...
  pthread_join(thidNew, (void **)NULL);
